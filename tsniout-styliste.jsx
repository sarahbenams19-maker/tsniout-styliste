...
Génère une tenue complète et tendance. Réponds UNIQUEMENT en JSON valide, sans backticks, sans markdown, sans texte avant ou après. Format exact :
{
  ...
  "pieces": [
    {
      ...
      "marque": "Nom exact de la marque parmi la liste autorisée",
      "prix_estime": "XX€",
      "pourquoi_tsniout": "Pourquoi cette pièce respecte les critères tsniout"
    }
  ],
  "conseil_styliste": "Un conseil personnel et chaleureux pour porter cette tenue avec confiance",
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Erreur API " + res.status + ": " + rawText.slice(0, 200));
      }
      let data;
      try { data = JSON.parse(rawText); } catch(e) { throw new Error("Réponse non-JSON: " + rawText.slice(0, 200)); }
      console.log("PARSED DATA:", JSON.stringify(data).slice(0, 500));
      let text = "";
      if (Array.isArray(data.content)) {
        ...
        throw new Error("Erreur IA: " + (data.error.message || JSON.stringify(data.error)));
      }
      console.log("TEXT EXTRACTED:", text.slice(0, 300));
      if (!text) throw new Error("Texte vide - structure: " + JSON.stringify(Object.keys(data)));
      const clean = text.replace(/```json\s*/gi, "").replace(/```\s*/gi, "").trim();
      const jsonMatch = clean.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error("JSON introuvable. Texte reçu: " + text.slice(0, 200));
  ...
  const reset = () => {
    setStep(0);
    setForm({ niveau: "", occasion: "", style: "", saison: "", budget: "moyen" });
    setResult(null);
    setError("");
  };
  ...
  return (
    <div style={S.wrap}>
      {/* HEADER */}
      <div style={{ textAlign: "center", padding: "48px 0 28px", width: "100%", maxWidth: 540 }}>
        <div style={{ fontSize: 12, letterSpacing: "0.3em", color: "#c9956a", marginBottom: 10, textTransform: "uppercase" }}>
          ✦ Styliste Personnelle ✦
        </div>
        <h1 style={{
          fontSize: "clamp(1.8rem, 7vw, 3rem)", fontWeight: 400, margin: 0, lineHeight: 1.1,
          background: "linear-gradient(135deg, #f5e6d3, #c9956a, #f5e6d3)",
          WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: "0.05em",
        }}>Tsnioût & Style</h1>
        <p style={{ color: "#a07860", fontSize: 14, marginTop: 10, letterSpacing: "0.05em" }}>
          Mode pudique • Tendance • Authentique
        </p>
      </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 28 }}>
          {[0,1,2,3,4].map(i => (
            <div key={i} style={{
              ...
              background: i < step ? "#c9956a" : i === step ? "#e8b080" : "rgba(201,149,106,0.2)",
